...
@router.post("/review-document", tags=["AI Agents"])
async def review_document(
    file: UploadFile,
    title: str = Form(...),
    evaluation_criteria: str = Form("General legal review"),
    user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    Review a legal document using AI and save the feedback to Supabase.
    
    Example form data:
    ```
    file: [legal_document.docx]
    title: "Contract Review"
    evaluation_criteria: "Check for potential liability issues and compliance with state laws"
    ```
    ...
    ```json
    {
        ...
        "feedback": "The document appears to be well-structured, but there are a few areas of concern..."
    }
    ```
    """
    try:
        logger.info(f"Reviewing document for user {user['id']} with title: {title}")
        user_id = user["id"]
        ...
        try:
            feedback = evaluate_legal_document(
                file_path=file_path,
                user_id=user_id,
                title=title,
                evaluation_criteria=evaluation_criteria
            )
            logger.info(f"Document review completed for {title}")
        except Exception as ai_error:
            logger.error(f"AI review failed: {str(ai_error)}\n{traceback.format_exc()}")
            raise HTTPException(
                status_code=500,
                detail=f"Failed to review document: {str(ai_error)}"
            )
        ...
        logger.error(f"Unexpected error in review_document: {str(e)}\n{traceback.format_exc()}")
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error: {str(e)}"
        )
